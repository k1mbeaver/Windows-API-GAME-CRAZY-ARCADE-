...
// 리소스로부터 읽어온다
HRESULT BitmapImage::init(const DWORD resID, int width, int height, bool isTrans)
{
    // 재초기화 방지용, 이미지 정보에 값이 있다면 릴리즈
    if (m_imageInfo != nullptr) release();
    // DC가져오기, 현재 윈도우에 대한 화면 DC를 임시적으로 받아옴
    ...
    m_imageInfo->hMemDC = CreateCompatibleDC(hdc);
    m_imageInfo->hBit = (HBITMAP)LoadBitmap(hInst, MAKEINTRESOURCE(m_imageInfo->resID)); // 파일을 비트맵에 올린다 
    m_imageInfo->hOBit = (HBITMAP)SelectObject(m_imageInfo->hMemDC, m_imageInfo->hBit);
    m_imageInfo->width = width;
    m_imageInfo->height = height;
...
// 이미지 설정 (파일로부터 읽어온다)
HRESULT BitmapImage::init(const char* fileName, int width, int height, bool isTrans)
{
    //파일경로가 잘못되었으면
...
// 이미지 + 프레임 파일 초기화
HRESULT BitmapImage::init(const char* fileName, int width, int height, int frameX, int frameY, bool isTrans)
{
    //파일경로가 잘못되었으면
    if (fileName == nullptr) return E_FAIL;
...
HRESULT BitmapImage::init(const char* fileName, int x, int y, int width, int height, int frameX, int frameY, bool isTrans)
{
    //파일경로가 잘못되었으면
    if (fileName == nullptr) return E_FAIL;
...
void BitmapImage::renderStretch(HDC hdc, int destX, int destY, int destWidth, int destHeight, int sourX, int sourY, int sourWidth, int sourHeight)
{
...
void BitmapImage::alphaRender(HDC hdc, int destX, int destY, BYTE alpha)
{
    m_blendFunc.SourceConstantAlpha = alpha;
    if (m_isTrans)
    {
...
void BitmapImage::alphaRender(HDC hdc, int destX, int destY, int sourX, int sourY, int sourWidth, int sourHeight, BYTE alpha)
{
    m_blendFunc.SourceConstantAlpha = alpha;
    if (m_isTrans)
    {
